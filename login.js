...
app.post("/userInfo", (request, response) => {
  ...
  addToDataBase(variables.name, variables.email, variables.pass).catch(
    console.error
  );
...
async function lookUp(user, response, pass) {
  try {
    await client.connect();
    await lookUpOneEntry(response, client, databaseAndCollection, user, pass);
  } catch (e) {
    console.error(e);
  } finally {
    await client.close();
...
async function lookUpOneEntry(
  ...
  if (!result) {
    response.send(
      `No user found with name <strong>${user}</strong><br><br><a href="/">HOME</a>`
    );
    return;
  }
  response.render("loggedIn", {
    userName: user,
    quotes: "Click the button to generate quotes",
  });
}
